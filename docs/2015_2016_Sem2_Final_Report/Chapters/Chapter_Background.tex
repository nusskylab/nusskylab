...
% Min: consider italicizing MVC components or other terminology that you introduce.  Reads and formats better (to me).
...
As is seen from the entity relation diagram shown in Figure~\ref{fig:SkylabER}, the users' basic information is captured in the \textit{User} model and each user may have one or more different roles such as \textit{Admin}, \textit{Student}, \textit{Adviser}, \textit{Mentor}, \textit{Tutor} and \textit{Facilitator}.
% Min: is this right?  A user cannot simultaneously be a tutor and adviser?
...
% Min: shouldn't it be called a survey and surveytemplate or feedback and feedbacktemplate?  
...
% Min: global: consider also using \textit here for controller names.
...
% Min: also use formatting for methods?
...
% Min: I don't understand this sentence
...
% Min: give URL for Travis, CodeClimate.  May need to introduce these services.
