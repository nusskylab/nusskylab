...
RSpec.describe Team, type: :model do
  ...
  it '#team_name custom validation test' do
    # first team is just to initialize and make the database non-empty 
    # so we can retrieve the next id 
    team = FactoryGirl.create(:team, team_name: 'first_team')    
    puts "start custome"
    next_id = Team.maximum(:id).next
    # test creation of team
    expect(FactoryGirl.create(:team, team_name: next_id.to_s)).to be_valid
    next_id += 1    
    
    # test with leading 0
    expect(FactoryGirl.create(:team, team_name: "0" + next_id.to_s)).to be_valid
    puts "end custom"
  end
