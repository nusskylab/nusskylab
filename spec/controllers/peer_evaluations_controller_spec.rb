  ...
  describe 'POST #create' do
    context 'user not logged in' do
      it 'should redirect to root_path for non_user' do
        milestone = FactoryGirl.create(
          :milestone, name: 'milestone_1.peerEvaluation.controller.spec')
        user0 = FactoryGirl.create(
          :user,
          email: '1@peerEvaluation.controller.spec',
          uid: '1.peerEvaluation.controller.spec'
        )
        adviser = FactoryGirl.create(:adviser, user: user0)
        team = FactoryGirl.create(
          :team,
          adviser: adviser,
          team_name: '1.peerEvaluation.controller.spec'
        )
        evaluated_team = FactoryGirl.create(
          :team, adviser: adviser,
          team_name: '2.peerEvaluation.controller.spec'
        )
        evaluating = FactoryGirl.create(
          :evaluating, evaluator: team, evaluated: evaluated_team
        )
        submission = FactoryGirl.create(
          :submission, team: evaluated_team, milestone: milestone)
        peer_evaluation_params = FactoryGirl.attributes_for(
          :peer_evaluation, submission_id: submission.id, team_id: team.id)
        post :create, team_id: team.id, milestone_id: milestone.id, questions: {
          '1': 'test'
        }, peer_evaluation: peer_evaluation_params
        expect(response).to redirect_to(root_path)
      end
    end
    ...
    context 'user logged in but not admin' do
      ...
      it 'should redirect to home_path with success for student' do
        ...
        adviser = FactoryGirl.create(:adviser, user: user0)
        team = FactoryGirl.create(
          :team, adviser: adviser, team_name: '1.peerEvaluation.controller.spec'
        )
        student = FactoryGirl.create(
          :student, team: team, user_id: subject.current_user.id
        )
        evaluated_team = FactoryGirl.create(
          :team, adviser: adviser, team_name: '2.peerEvaluation.controller.spec'
        )
        evaluating = FactoryGirl.create(
          :evaluating, evaluator: team, evaluated: evaluated_team
        )
    ...
    context 'user logged in and student' do
      ...
      it 'should redirect to new_milestone_team_peer_evaluation_path with danger for student' do
        ...
        peer_evaluation = FactoryGirl.create(
          :peer_evaluation, submission: submission, team: team
        )
        peer_evaluation_params = FactoryGirl.attributes_for(
          :peer_evaluation, submission_id: submission.id, team_id: team.id
        )
        ...
        expect(response).to redirect_to(
          new_milestone_team_peer_evaluation_path(milestone, team)
        )
  ...
  describe 'PUT #update' do
    context 'user not logged in' do
      it 'should redirect to root_path for non_user' do
        ...
        put :update, id: peer_evaluation.id, team_id: team.id, questions: {
          '1': 'test'
        }, milestone_id: milestone.id, peer_evaluation: peer_evaluation_params
        expect(response).to redirect_to(root_path)
      end
    end
